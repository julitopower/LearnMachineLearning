...
class DummyAgent(ActorCriticEngine):
    ...
    def build_model(self):
        # We can't use the Sequential API, because the model now has 2 outputs
        inputs = keras.Input(shape=(self.state_dim))

        prev = inputs
        for layer in self.layers:
            lyer = keras.layers.Dense(layer, activation='relu',
                                      kernel_initializer='glorot_normal')(prev)
            prev = lyer

        # Output of the Actor is an action    
        actor = keras.layers.Dense(self.action_dim, activation='softmax')(prev)
        # Output of the critic is the expected reward from the current state
        critic = keras.layers.Dense(1)(prev)
